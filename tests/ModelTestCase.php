...
abstract class ModelTestCase extends TestCase
{
     ...
     * - `getOwnerKey()`: `BelongsTo` relation and its extendings.
     */
    protected function assertBelongsToManyRelation($relation, Model $model, Model $related, $pivotForeignKey, $relateForeignKey, $owner = null, $relateKey = null, \Closure $queryCheck = null)
    {
        ...
        if (is_null($pivotForeignKey)) {
            $pivotForeignKey = $model->getForeignKey();
        }
        $this->assertEquals($pivotForeignKey, $relation->getForeignPivotKeyName());
        if (is_null($owner)) {
            $owner = $model->getKeyName();
        }
        $this->assertEquals($owner, $relation->getParentKeyName());
        if (is_null($relateForeignKey)) {
            $relateForeignKey = $related->getForeignKey();
        }
        $this->assertEquals($relateForeignKey, $relation->getRelatedPivotKeyName());
        if (is_null($relateKey)) {
            $relateKey = $related->getKeyName();
        }
        $this->assertEquals($relateKey, $relation->getRelatedKeyName());
    }
}
